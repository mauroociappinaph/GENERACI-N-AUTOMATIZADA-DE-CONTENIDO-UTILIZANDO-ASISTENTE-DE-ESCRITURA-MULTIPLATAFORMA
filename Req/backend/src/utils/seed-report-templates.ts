...
export async function seedReportTemplates(): Promise<void> {
  try {
    ...
    if (!adminUser) {
      logger.warn('No admin user found, skipping report template seeding');
      return;
    }
    ...
    for (const template of reportTemplates) {
      ...
      if (!existingTemplate) {
        ...
        logger.info(`Report template already exists: ${template.name}`);
      }
    }

    logger.info('Report template seeding completed');
  } catch (error) {
